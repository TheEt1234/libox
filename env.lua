...
function libox.safe.PcgRandom(seed, seq)
	if seq and #seq > 1000 then
		error("Sequence too large, size limit is 1000", 2)
	end
...
function libox.safe.PerlinNoise(noiseparams)
	if type(noiseparams) ~= "table" then
		return false, "noiseparams is a table, deprecated syntax is not supported."
	end
...
function libox.safe.get_loadstring(env) -- chunkname is ignored
	return function(code, chunkname)
		...
		if type(code) == "string" and #code > 64000 then
			error("Code too long :/", 2)
		end
...
function libox.create_basic_environment()
	...
	local env = {
		...
		collectgarbage = function(arg)
			if arg ~= "count" then
				error("The only valid mode to collectgarbage is count")
			end
